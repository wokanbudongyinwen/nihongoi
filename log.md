...
## 首页双页手札与沉浸式布局（2026-09-02）

- `pages/index/index.uvue`：学习与工具拆为原生 swiper 两页，底部分页可点按；整页单纸卡、轻量分区、卡内纵向滚动、上下安全区。保留学习、统计下钻和四个工具路由，不改数据逻辑。
- `pages.json`：仅首页启用自定义导航，移除顶部原生“今日”标题栏。
- `DESIGN.md`、`AGENTS.md`：同步双页结构与交互规则。
- 验证：HBuilderX 5.24 Android 编译成功；MuMu 1080×1920 两页截图、横滑/点按分页、四个工具跳转及返回保留页签、统计总词数下钻通过。临时 1080×1500 短屏确认卡内可纵向滚动，随后恢复原始分辨率；显式绑定 scroll-y=true 修复裸属性未开启滚动的问题。
